...
Mat HLSdetetion::PreFix(const Mat &src)
{
	//imshow("Camera", src);
	int iIgnoreObj = 1;
...
Mat HLSdetetion::ThresholdDetection(Mat &draw, const Mat &Thres)
{
	RNG rng(12345);
...
int HLSdetetion::CheckSign(const Mat &src)
{
	if (flag)
	{
		if (HLSdetetion::CountFrame > HLSdetetion::UnlockFlag)
		{
			HLSdetetion::CountFrame = 0;
			HLSdetetion::flag = false;
		}
		else
		{
			HLSdetetion::CountFrame++;
			return HLSdetetion::OldSign;
		}
	}
	...
	if (HLSdetetion::CountFrame > HLSdetetion::LockFlag)
	{
		// confim traffic sign
		flag = true;
		CountFrame = 1;
