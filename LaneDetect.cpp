...
void LaneDetect::FindLane(const Mat &BinarySrc)
{
	...
	for (int high = BinarySrc.rows - Box.height; high > 20; high = high - (Box.height >> 1)) // with a line
	{
		for (int i = MidLane - IgnoreFromMid + (high - 10 >> 1); i > Box.width; i = i - (Box.width >> 1))
		{
		...
		for (int i = MidLane + IgnoreFromMid - (high - 10 >> 1); i < BinarySrc.cols - Box.width; i = i + (Box.width >> 1))
		{
			Mat box = Mat(BinarySrc, Rect(i, high, Box.width, Box.height));
			if (countNonZero(box) * 100 > Box.area()*Accuracy)
			{
...
void LaneDetect::DrawLane()
{
	for (int i = 0; i < LaneL.size(); i++)
		circle(draw, LaneL[i], 2, Scalar(255, 0, 0), -1);
	for (int i = 0; i < LaneR.size(); i++)
		circle(draw, LaneR[i], 2, Scalar(0, 255, 0), -1);
