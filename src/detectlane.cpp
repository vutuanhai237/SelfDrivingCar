...
//Add new     fsfsfsfs
void DetectLane::Def_Thres()
{
    minThreshold[0] = 50;
    minThreshold[1] = 0;
    minThreshold[2] = 150;
}
void DetectLane::Fix_Thres()
{
    minThreshold[0] = 50;
    minThreshold[1] = 0;
    minThreshold[2] = 100;
...
void DetectLane::update(const Mat &src)
{
    ...
    for (int i = 1; i < (rightLane.size() + leftLane.size())/2; i++)
    {
        
        if (rightLane[i] != null && leftLane[i] != null) {
            Point tmp = (leftLane[i]+ rightLane[i])/2;
            circle(lane, tmp, 1, Scalar(0,255,0), 2, 8, 0 );
             pre_tmp  = tmp;
             lane_width2 = (lane_width2 + rightLane[i].x - leftLane[i].x) / 2;
        }
        else
        {
            if (rightLane[i] != null)
            {
                Point tmp;                
                tmp.y = 5;
                tmp.x = rightLane[i].x - int(lane_width2) * 1 / 2 + 20;
                circle(lane, tmp, 1, Scalar(0,255,0), 2, 8, 0 );
                 pre_tmp  = tmp;
            }
            else if  (leftLane[i] != null)
            {
                Point tmp;               
                tmp.y = 5;
                tmp.x = leftLane[i].x + int(lane_width2) * 1 / 2 - 20;
                circle(lane, tmp, 1, Scalar(0,255,0), 2, 8, 0 );
                 pre_tmp  = tmp;
            }
            else
            {
                circle(lane, pre_tmp, 1, Scalar(0,255,0), 2, 8, 0 );
            }          
        }       
    }
...
Mat DetectLane::birdViewTranform(const Mat &src)
{
    ...
    src_vertices[0] = Point(0, skyLine+20);
    src_vertices[1] = Point(width, skyLine+20);
