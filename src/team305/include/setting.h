...
const int TrafficSign::LockFlag = 5; // 7 frame - after 10 frame detect a same sign, lock that result and stop detect
const int TrafficSign::UnlockFlag = 90; // ~2s
...
int LaneDetect::Accuracy = 35; // 55%

double LaneDetect::Fx_a = double(5) / 13; //F(x) = ax + b; a = 5/13 ////6/13
double LaneDetect::Fx_b = double(395) / 13; //F(x) = ax + b; b = 395/13 ///370/13 

int LaneDetect::start = 0;
const int LaneDetect::TooFew = 5; // The value decition when too few point to determine a lane
...
const int CarControl::DefMaxSpeed = 70; //65
